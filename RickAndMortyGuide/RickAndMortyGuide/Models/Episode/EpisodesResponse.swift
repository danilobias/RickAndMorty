...
public class EpisodesResponse: NSCoding {
    ...
    // MARK: Properties
    public var results: [EpisodeResult]?
    ...
    public required init(json: JSON) {
        ...
        if let items = json[SerializationKeys.results].array {
            results = items.map { EpisodeResult(json: $0) }
        }
    ...
    required public init(coder aDecoder: NSCoder) {
        self.results = aDecoder.decodeObject(forKey: SerializationKeys.results) as? [EpisodeResult]
