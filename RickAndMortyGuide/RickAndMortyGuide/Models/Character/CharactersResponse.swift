...
public class CharactersResponse: NSCoding {
    ...
    public required init(json: JSON) {
        ...
        if let items = json[SerializationKeys.results].array {
            results = items.map { CharacterResult(json: $0) }
        }
    ...
    required public init(coder aDecoder: NSCoder) {
        self.results = aDecoder.decodeObject(forKey: SerializationKeys.results) as? [CharacterResult]
